"""Page that shows the raw url database."""
...
layout = html.Div([
    dbc.Row([
        layout_menu.layout,
        dbc.Col(id='url_update_time'),
        dbc.Col(id='url_data'),
        dbc.Col(
            html.Button(
                "Update table", id='url_refresh_button',
                style={
    ...
    dbc.Row([
        dcc.Graph(id='url_table', style={'width': '100%'})
    ], style={
        'margin-left': f"{CFG['SIDE_PADDING']}px",
        'margin-right': f"{CFG['SIDE_PADDING']}px",
        'margin-bottom': f"{CFG['DIVISION_PADDING']}px"
...
@callback(
    Output('url_table', 'figure'),
    Output('url_update_time', 'children'),
    Output('url_data', 'children'),
    Input('url_refresh_button', 'n_clicks'))
def update_url(_1):
    """Makes url graph."""
    global CFG
    CFG = load_config()
    dataframe = load_dataframe('urls')
